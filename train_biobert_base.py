...
for fold, (train_idx, val_idx) in enumerate(kf.split(texts), 1):
    ...
    # 3. 初始化 Tokenizer + Model (BioBERT-Base)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        classifier_dropout=0.2  # BioBERT-Base較小，降低dropout
    )
    
    # 4. Full Fine-tuning - 不凍結任何層
    freeze_n = 0  # 全部微調
    print(f"Fold {fold} - 凍結層數: {freeze_n} (Full Fine-tuning)")
    ...
    # 7. Trainer 設定 - 針對 BioBERT-Base 優化
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=24,      # 增加batch size（Base模型較小）
        gradient_accumulation_steps=2,       # 有效batch size = 48
        per_device_eval_batch_size=32,       # 評估時可以更大
        learning_rate=2e-5,                  # 適中的學習率
        num_train_epochs=15,                  # 適中的epochs
        weight_decay=0.03,                   # 適度的權重衰減
        logging_dir=logging_dir,
        logging_steps=25,                    # 更頻繁的logging
        save_total_limit=1,
        ...
        no_cuda=False,
        fp16=True,                          # 使用混合精度
        warmup_ratio=0.15,                  # 更多的warmup
        metric_for_best_model="eval_f1",    # F1 score為準
        greater_is_better=True,
        load_best_model_at_end=True,
        eval_strategy="epoch",
        save_strategy="epoch",
        dataloader_num_workers=2,           # 加速數據載入
    ...
    trainer = ImprovedTrainer(
        ...
        use_focal_loss=False  # 可以改為True嘗試focal loss
    )
    
    # 8. 開始訓練
    print(f"開始訓練 Fold {fold} (BioBERT-Base Full Fine-tuning)...")
    trainer.train()
...
plt.xlabel('Fold')
plt.ylabel('Score')
plt.title('BioBERT-Base 5-Fold Cross Validation Results')
plt.legend()
plt.grid(True)

plt.tight_layout()
plt.savefig('plots/biobert_base_cross_validation_results.png', dpi=300, bbox_inches='tight')
plt.close()

print("\nBioBERT-Base cross validation results saved to plots/biobert_base_cross_validation_results.png")